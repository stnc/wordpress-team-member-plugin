<?php
define( 'Nuc_team_PATH', plugin_dir_path( __FILE__ ) . 'includes/' );
...
function Nuc_register_post_type_team() {
	$singular = 'team';
...
add_action( 'init', 'Nuc_register_post_type_team' );


require( Nuc_team_PATH . "class-team-member.php" );
...
$Nuc_team_ch_postID = isset( $_GET['post'] ) ? $_GET['post'] : null;//post  id  for edit

$Nuc_team_post_type_ch = get_post_type( $Nuc_team_ch_postID );//get type

$Nuc_team_post_type_post = isset( $_REQUEST['post_type'] ) ? $_REQUEST['post_type'] : 'post';//for new

if ( $Nuc_team_post_type_post == 'team' or $Nuc_team_post_type_ch == 'team' ) {
	if ( is_admin() ) {
		add_action( 'load-post.php', 'Nuc_team_init_metabox' );
		add_action( 'load-post-new.php', 'Nuc_team_init_metabox' );
	}
}
...
function Nuc_schedule_team_expiration_event_insert( $post_id ) {
	...
	Nuc_updateCategories_team_after_expiration( $post_id );
	write_log("run");

}


add_action( 'Nuc_updateCategories_team_after_expiration', 'Nuc_updateCategories_team_after_expiration', 10, 1 );
// This function will run once the 'addCategories_team_after_expiration' is called


function Nuc_updateCategories_team_after_expiration( $post_id ) {
...
//buranın amacı term daha oluşmuş mu ona bakar eğer oluşmuşsa oluşturmaz
	...
	$selected_departman_id = Nuc_get_meta( $post_id, 'display_doctor_department', 'Nuc_DoctorAndDepartmant_ForSingleteamPage' );
...
//secilen departmanın bulur
	...
	$daha_once_eklene_degerler = get_post_meta( $selected_departman_id, 'Nuc_mpevent_departmentForSingleEventPage' );
	...
	update_post_meta( $selected_departman_id, 'Nuc_mpevent_departmentForSingleEventPage', $daha_once_eklene_degerler );
}


//  runs when a Post is update
add_action( 'publish_team', 'Nuc_team_schedule_team_expiration_event_update' );
function Nuc_team_schedule_team_expiration_event_update( $post_id ) {
	// Schedule the actual event
	//updateCategories_team_after_expiration( $post_id );
	wp_schedule_single_event( strtotime( "+2 seconds" ), 'Nuc_updateCategories_team_after_expiration', array( $post_id ) );
}
...
add_action( 'pre_post_update', 'Nuc_post_updating_callback' );
function Nuc_post_updating_callback( $post_id ) {
	global $post;
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
